...
class Ovis(OvisPreTrainedModel):
    ...
    def merge_multimodal(
        ...
        if self.training:  # padding to self.config.multimodal_max_length for increased training speed
            ...
            input_embeds[0] = torch.nn.ConstantPad2d((0, 0, padding_size, 0), 0.0)(input_embeds[0])
            attention_masks[0] = torch.nn.ConstantPad1d((padding_size, 0), False)(attention_masks[0])
            labels[0] = torch.nn.ConstantPad1d((padding_size, 0), IGNORE_ID)(labels[0])
        batch_input_embeds = self.pad_truncate_sequence(input_embeds, batch_first=True, padding_value=0.0, padding_side=padding_side)
        batch_attention_mask = self.pad_truncate_sequence(attention_masks, batch_first=True, padding_value=False, padding_side=padding_side)
        batch_labels = self.pad_truncate_sequence(labels, batch_first=True, padding_value=IGNORE_ID, padding_side=padding_side)

        return visual_input_ids, batch_input_embeds, batch_labels, batch_attention_mask

    def pad_truncate_sequence(self, sequences: List[torch.Tensor], batch_first: bool = True, padding_value: float = 0.0, padding_side: str = "left") -> torch.Tensor:
        pad_sequence = torch.nn.utils.rnn.pad_sequence(sequences, batch_first=batch_first, padding_value=padding_value, padding_side=padding_side)
